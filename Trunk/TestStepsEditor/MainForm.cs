...
namespace TestStepsEditor
{
    public partial class MainForm : Form
    {
        ...
        private SelectQuery _queryPicker;
        private SelectTestCase _testCasePicker;

        public IEnumerable<ITestCase> TestCasesFromQuery
        {
            get
            {
        ...
        public string TestCaseNumber
        {
            set
            {
                this._workItemIdToolStripComboBox.Text = value;
        ...
        private void CloseCurrentTab()
        {
            ...
            {
                this._logger.Info("Close current test: dirty.");
        ...
        {     
            if (this._queryPicker == null || this._queryPicker.Project != this._testProject)
            {
                this._queryPicker = new SelectQuery(this) { Project = this._testProject };
            }

            this._queryPicker.StartPosition = FormStartPosition.Manual;
            this._queryPicker.Location = new Point(this.Location.X + 25, this.Location.Y + 25);
            this._queryPicker.ShowDialog();
            if (this._testCases == null || this._testCases.LongCount() <= 0)
            {
                return;
            }

            this._testCasePicker = new SelectTestCase(this)
                {
                    StartPosition = FormStartPosition.Manual, 
                    Location = new Point(this.Right + 10, this.Top)
                };
            this._testCasePicker.Show();
        }
        ...
        private void RefreshCurrentTestCaseToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            this.TestCaseNumber = this._testTabControl.SelectedTab.Text.Split(' ')[0].Trim();
            this.CloseCurrentTab();
            this.GetTestCaseData();
        }
    }
